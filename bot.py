...
@tree.command(name="whisptranslate", description="✨ Translate the message you're replying to, just for you.")
async def slash_translate(interaction: discord.Interaction):
    if not interaction.channel or not interaction.data.get("message_reference"):
        await interaction.response.send_message("🌸 Please use this command by replying to the message you want translated.", ephemeral=True)
        return
    ...
    try:
        ref = interaction.data.get("message_reference")
        try:
            original_msg = await interaction.channel.fetch_message(ref["message_id"])
        content = original_msg.content
        if not content:
            await interaction.response.send_message("🧺 That message carries no words to whisper.", ephemeral=True)
            return
    except Exception as e:
        print("Fetch error:", e)
        await interaction.response.send_message("❗ I couldn’t find the message you replied to.", ephemeral=True)
        return
    ...
    if not user_lang:
        await interaction.response.send_message("🕊️ You haven’t chosen a language yet, friend.", ephemeral=True)
        return

    # Optional: Trigger glitch mode
    maybe_glitch = maybe_trigger_glitch(guild_id)
    current_mode = guild_modes[guild_id]
